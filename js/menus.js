define( /* Menus */ 
["jquery", "character", "character-class", "character-growth", "equipment", "messages"], 
function($, Character, CharacterClass, CharacterGrowth, Equipment, Message) {
  return (function() {
    ...
    var loadNames = function(chars, $container) {
    ...
    self.Char = (function() {
      ...
      // PRIVATE METHODS 
      var initCharacters = function(chars) {
      ...
      self.init = function() {
        ...
        var markup = [];
        var m = 0;
        for (var c = 0; c < Character.MAX_CHARS; c++) {
          m = 0;
      ...
      self.load = function(chars, orbsLit, gp) {
        for (var i = 0; i < orbsLit.length; i++) {
          $container.find(".orb." + orbsLit[i]).addClass("lit");
        }
        $container.find(".gold").empty().append(Message.create(Message.padToLength(gp, 6) + " G"));
    ...
    self.Armor = (function() {
      ...
      self.load = function(chars) {
        loadNames(chars, $container);
        
        $container.find(".char").each(function(i) {
          self.reloadChar(chars[i], $(this));
        });
      };
    ...
    self.Magic = (function() {
      ...
      self.load = function(char) {
        $container.find(".name").empty().append(Message.create(char.getName()));
    ...
    self.Item = (function() {
      ...
      self.init = function() {
        $container = $("#itemMenu");
        $container.find(".title").append(Message.create("ITEM"));
      };
      
      self.load = function(consumables, keyItems) {
        ...
        $.each(keyItems, function(i, keyItem) {
          $inventory.append($("<div/>").addClass("item").append(Message.create(keyItem.name)));
        });
      };
    ...
    self.NewChar = (function() {
      ...
      var defaultSelections = function() {
        $container.find(".slot").each(function(i, slot) { self.selectionChanged($(slot), i); });
      };
