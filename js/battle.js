...
function($, Logger, Monster, Party, RNG) {
  ...
  var calculateSurprise = function(battle) {
    ...
    if (!battle.runnable) {
      Logger.debug(LOG_ID, "encounter is not runnable, normal battle");
      return;
    }
    ...
    Logger.info(LOG_ID, "SURPRISE = (leader quick + random - surprise) " + leaderQuickness + " + " + r + " - " + battle.surprise + " = " + result);
  ...
  Battle.prototype.isMixedSize = function() {
    var numSizes = 0;
    if (this.enemiesBySize.small && this.enemiesBySize.small.enemies.length > 0) { numSizes++; }
    if (this.enemiesBySize.large && this.enemiesBySize.large.enemies.length > 0) { numSizes++; }
    if (this.enemiesBySize.fiend && this.enemiesBySize.fiend.enemies.length > 0) { numSizes++; }
    return numSizes > 1;
  };
