...
function(Map, MapConstants, MovementConstants) {
  ...
  var tiles = {
    ...
    "r1": Map.newTile({y:0, x:4, desc:"road top left"}).passableBy(Transport.Foot),
    "r2": Map.newTile({y:1, x:4, desc:"road bottom left"}).passableBy(Transport.Foot),
    "r3": Map.newTile({y:1, x:5, desc:"road bottom right"}).passableBy(Transport.Foot),
    "r4": Map.newTile({y:0, x:6, desc:"road shadow top"}).passableBy(Transport.Foot),
    "r5": Map.newTile({y:1, x:6, desc:"road shadow bottom"}).passableBy(Transport.Foot),
    "t" : Map.newTile({y:1, x:10, desc:"tree"}),
    "tt": Map.newTile({y:0, x:10, desc:"trees"}),
    "w" : Map.newTile({y:0, x:8, desc:"water"}),
    ...
    "IN": Map.newTile({y:0, x:21, desc:"inn shop sign"}),
    "IT": Map.newTile({y:0, x:20, desc:"item shop sign"}),
    "WM": Map.newTile({y:0, x:19, desc:"white magic shop sign"}),
    "WP": Map.newTile({y:0, x:18, desc:"weapon shop sign"}),
    "Bh": Map.newTile({y:1, x:14, desc:"bridge horizontal"}).passableBy(Transport.Foot),
    "Bv": Map.newTile({y:0, x:14, desc:"bridge vertical"}).passableBy(Transport.Foot),
    "D" : Map.newTile({y:1, x:16, desc:"door"}).passableBy(Transport.Foot),
    "F" : Map.newTile({y:1, x:21, desc:"fountain"}),
    "FL": Map.newTile({y:0, x:12, desc:"flowers"}).passableBy(Transport.Foot),
    "FN": Map.newTile({y:0, x:11, desc:"fence"}),
    "G" : Map.newTile({y:1, x:11, desc:"gravestone"}),
    "H1": Map.newTile({y:1, x:17, desc:"house side single window"}),
    "H2": Map.newTile({y:0, x:17, desc:"house side double window"}),
    "Rt": Map.newTile({y:0, x:15, desc:"roof top"}),
    "Rb": Map.newTile({y:1, x:15, desc:"roof bottom"}),
    "R^": Map.newTile({y:0, x:16, desc:"roof angle"}),
    "S" : Map.newTile({y:1, x:12, desc:"sand"}).passableBy(Transport.Foot),
