...
function(Map, MapConstants, MovementConstants) {
  ...
  var tiles = {
    ...
    "v" : Map.newTile({y:2, x:5}).desc("stairs down").passableBy(Transport.Foot),
    "[]": Map.newTile({y:2, x:4}).desc("door").inside({y:5, x:4}).passableBy(Transport.Foot).roomEntry(),
    ...
    "$" : Map.newTile({y:1, x:1}).desc("chest").inside({y:5, x:3}).passableBy(Transport.Foot),
    ...
    "^^": Map.newTile({y:1, x:1}).desc("arrows").inside({y:6, x:0}).passableBy(Transport.Foot),
    "@" : Map.newTile({y:1, x:1}).desc("orb altar").inside({y:6, x:1}).passableBy(Transport.Foot),
    "O" : Map.newTile({y:1, x:1}).desc("orb").inside({y:0, x:0}),
    "*" : Map.newTile({y:1, x:1}).desc("no idea").inside({y:6, x:4}),
    "@@": Map.newTile({y:1, x:1}).desc("transporter").inside({y:3, x:4}).passableBy(Transport.Foot),
